...
public class CompleteBookingCommandHandler : IRequestHandler<CompleteBookingCommand, bool>
{
    ...
    public CompleteBookingCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
    ...
    public async Task<bool> Handle(CompleteBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (booking.Status != BookingStatus.CheckedOut)
            throw new InvalidOperationException("Chỉ có thể Hoàn tất cho đơn đặt phòng đã Check-out.");
