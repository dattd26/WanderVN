...
public class CheckOutBookingCommandHandler : IRequestHandler<CheckOutBookingCommand, bool>
{
    ...
    public CheckOutBookingCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
    ...
    public async Task<bool> Handle(CheckOutBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (booking.Status != BookingStatus.CheckedIn)
            throw new InvalidOperationException("Chỉ có thể Check-out cho đơn đặt phòng đã Check-in.");
