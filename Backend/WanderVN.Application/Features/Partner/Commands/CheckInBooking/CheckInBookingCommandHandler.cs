...
public class CheckInBookingCommandHandler : IRequestHandler<CheckInBookingCommand, bool>
{
    ...
    public CheckInBookingCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
    ...
    public async Task<bool> Handle(CheckInBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (booking.Status != BookingStatus.Confirmed)
            throw new InvalidOperationException("Chỉ có thể Check-in cho đơn đặt phòng đã Xác nhận.");
